...
def calculate_pearsonr(all_predictions, all_labels):
    pearson_vals = []
    for i in range(all_labels.shape[0]):
        pearson_vals.append(pearsonr(all_labels[i], all_predictions[i])[0] if pearsonr(all_labels[i], all_predictions[i])[1] < 0.05 else None)
